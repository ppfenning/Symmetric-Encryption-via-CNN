...
def __transform_encryption(data):
    return np.bitwise_xor.reduce(np.mod(np.floor(np.abs(data) * 10 ** 10), 2**16).astype(np.uint16), axis=1)
...
def get_encryption(audio, *, throw_away, henon_0, ikeda_0, lorenz_0, logistic_0):
    ...
    encryption = __transform_encryption(np.concatenate((
    ...
    return np.bitwise_xor.reduce(np.array([audio, encryption], dtype=np.uint16)).astype(np.int16)
